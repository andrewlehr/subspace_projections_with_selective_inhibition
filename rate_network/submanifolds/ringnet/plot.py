...
class Plot:
    ...
    def activity_raster(self, R, storage_loc=None, figsize=(6,3), title=None, xlabel='time step', labelsize=18):
        fig, ax = plt.subplots(figsize=figsize)
        im = ax.imshow(R, aspect='auto', cmap='Greys', origin='lower')
        cb = plt.colorbar(im)
        cb.ax.tick_params(labelsize=self.tick_size)
        ...
        plt.ylabel('neuron', fontsize=labelsize)
        
        if title != None:
            plt.title(title, fontsize=labelsize)
        
        if storage_loc != None:
            plt.savefig(storage_loc+'.png', dpi=200, bbox_inches="tight")
            plt.savefig(storage_loc+'.pdf', bbox_inches="tight")
